...
type AdminCustomers =
    Awaited<ReturnType<typeof fetchAdminCustomers>>["data"];

type AdminDeals =
    Awaited<ReturnType<typeof fetchAdminDeals>>["data"];
...
export function AdminPage() {
    ...
    const isPageLoading =
        customersQuery.isLoading || dealsQuery.isLoading;
    ...
    return (
        <div className="space-y-8">
            <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="font-display text-2xl text-forest-950">
                        Administration
                    </h1>
            ...
            <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h2
                        ...
                        className="text-lg font-medium text-forest-950"
                    >
                        Kunder
                    </h2>
        ...
        <section aria-labelledby="deals-heading">
            <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-forest-900/60">
                        Senaste registrerade kundaffärer.
                    </p>
                </div>
                            ...
                            <Card>
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="truncate font-medium text-forest-950">
                                            {deal.title}
                                        </p>

                                        <p className="mt-1 truncate text-xs text-forest-900/60">
                                            {deal.externalId} ·{" "}
                                            {deal.customer.user.name}
                                        </p>
                                    </div>
                                ...
                                <div className="mt-4 border-t border-forest-100 pt-3">
                                    ...
                                    <p className="mt-1 font-medium text-forest-950">
                                        {formatSek(deal.estimatedGrossSek)}
                                    </p>
                                </div>
                            </Card>
                        </li>
...
function IntegrationStatus() {
    return (
        <section className="rounded-xl border border-forest-200 bg-white p-4">
            <h2 className="mb-3 text-lg font-medium text-forest-950">
                Integrationer
            </h2>

            <ul className="space-y-3 text-sm">
                <StatusRow
                    label="VSYS (skogsägarsystem)"
                    ok
                    note="Senast: 12 min sedan"
                />

                <StatusRow
                    label="SDC / VMF mätdata"
                    ok
                    note="Senast: 4 tim sedan"
                />

                <StatusRow
                    label="Bank (SEB)"
                    ok
                    note="Senast: i natt kl. 02:00"
                />
...
function CreateDealModal({
    ...
    const [dealType, setDealType] =
        useState<DealType>("REGENERATION_FELLING");
    ...
    function submit(event: FormEvent<HTMLFormElement>) {
        ...
        if (
            ...
            (!Number.isFinite(estimatedGrossSek) ||
                estimatedGrossSek < 0)
        ) {
            return;
        }
        ...
        >
            ...
            >
                <div className="mb-4 flex items-center justify-between gap-3">
                    <h3
                        ...
                        className="font-display text-lg text-forest-950"
                    >
                        Skapa ny affär
                    </h3>
            ...
            <span className="mb-1 block font-medium">
              Typ
                        ...
                        <select
                            ...
                            onChange={(event) =>
                                setDealType(
                                    event.target.value as DealType,
                                )
                            }
                            disabled={
                                configQuery.isLoading ||
                            ...
                            className="w-full rounded-lg border border-forest-200 bg-forest-50 px-3 py-2 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            {(configQuery.data?.dealTypes ?? []).map(
                                (type) => (
                                    <option
                                        key={type}
                                        value={type}
            ...
            <span className="mb-1 block font-medium">
              Preliminärt brutto (SEK)
                    ...
                    {configQuery.error && (
                        <ErrorMessage error={configQuery.error} />
                    )}
...
function EmptyState({
    ...
    return (
        <div className="rounded-xl border border-dashed border-forest-300 bg-forest-50/40 px-6 py-10 text-center">
            ...
            <h3 className="font-medium text-forest-950">
                {title}
            </h3>

            <p className="mx-auto mt-1 max-w-md text-sm text-forest-900/60">
                {description}
            </p>
        </div>
    );
