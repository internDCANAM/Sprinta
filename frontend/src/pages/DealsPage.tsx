  ...
  const grouped = useMemo(() => {
    ...
    for (const d of deals) {
      if (ONGOING.has(d.status)) {
        ongoing.push(d);
        totalEstimated += d.estimatedGrossMinor ?? 0;
      } else {
        finished.push(d);
      }
    }
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="mt-2 text-sm opacity-90">
          {grouped.ongoing.length} aktiv{grouped.ongoing.length === 1 ? '' : 'a'} affär
          {grouped.ongoing.length === 1 ? '' : 'er'}
        </p>
      </section>
...
function DealCard({ deal, onClick }: { deal: DealSummary; onClick: () => void }) {
  const amount = deal.finalGrossMinor ?? deal.estimatedGrossMinor;
  return (
    <Card onClick={onClick}>
      ...
      <div className="mt-3 flex items-baseline justify-between">
        <span className="text-xs text-forest-900/60">
          {deal.finalGrossMinor ? 'Slutlikvid' : 'Preliminärt'}
        </span>
        <span className="font-display text-lg text-forest-900">{formatMoney(amount)}</span>
      </div>
    </Card>
  );
}
