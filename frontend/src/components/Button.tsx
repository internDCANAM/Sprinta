...
const BASE = 'inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50';
...
export function Button({ variant = 'primary', block, className, ...rest }: Props) {
  return (
    <button className={clsx(BASE, VARIANTS[variant], block && 'w-full', className)} {...rest} />
  );
}
